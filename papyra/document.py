...
class PdfDocument(QObject):
    ...
    def text_spans(self, index: int, textpage=None) -> list[TextSpan]:
        """Trechos de texto (um por 'span' já segmentado pelo PyMuPDF — ou
        seja, uma sequência de caracteres com a mesma fonte/tamanho/cor).
        É a unidade usada para editar texto existente: clicar em um trecho
        edita só aquele trecho, sem tocar em texto vizinho com outro estilo."""
        page = self._doc[index]
        raw = page.get_text("dict", textpage=textpage) if textpage is not None else page.get_text("dict")
        spans: list[TextSpan] = []
    ...
    def ocr_text_spans(self, index: int, language: str = "por+eng", dpi: int = 300) -> list[TextSpan]:
        """Reconhece o texto de uma página digitalizada via OCR (Tesseract) e
        devolve trechos editáveis (um por estilo) nas mesmas coordenadas da
        página real.
        ...
        return self.text_spans(index, textpage=textpage)
